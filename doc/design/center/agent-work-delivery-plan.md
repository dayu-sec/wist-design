...
| 14 | **资产清单从哪里算** | **已定在中心**（`models/software-normalization-and-vuln-enrichment.md` §2：边缘只发现线索，归一化与漏洞 enrichment 在中心）。三条判据、分工与**缺的接口（机器台账如何到中心）**见 §8.2 |
...
### 8.2 资产清单从哪里算（已定在中心；缺一个接口）

**判据三条**（不是拍脑袋）：

1. **知识库的体积与变更频率决定它放哪**。归一化要 CPE / purl / SWID / 内部 `software_id` + 版本解析规则 —— 大且**高频变**（漏洞情报天天变），不能下发到 N 台机器。对照用途规则表：43 条、稳定 → 放网关。
2. **输入不能残缺**。摘要是压缩产物：去重后只剩「进程可执行标识 + 包名 + 监听端口」，**丢了 pid / cmdline / target 关系 / 按方向的完整属性** —— 而清单要的恰是这些（“这个端口是哪个进程在听”）。**在摘要上算清单 = 在残缺输入上算**；要算就得走**原文快照**（按设计就是给中心的）。
3. **「清单」本身就是跨机器的**。一台机器只能算出「这台机器上有什么」，清单是全局集合视图：同一软件在多台机器上 = **一条资产 + N 个实例**，还要跨机去重、生命周期（出现/消失/升级）、关联漏洞。这些要**全局视图 + 历史**，网关（单机、覆盖式、无历史）**结构上**做不到，不是性能问题。

| 层 | 出什么 | 形态 |
|---|---|---|
| agentd | 事实**线索**（resources / targets） | 原文快照 |
| **网关** | **单机视图 + 推断**（“这台机器是干什么用的”、“这台机器上有什么”） | 覆盖式状态 |
| **中心** | **资产清单**（归一化 → 软件实体 → 生命周期 → 漏洞关联） | 有历史的目录 |

**缺的接口**：清单需要**机器台账**做左表（“我管的机器有哪些”），而台账在网关（`agents` 表）。中心要么从网关同步台账，要么自己也维护一份 —— **现在没有任何设计**（现有文档只说“中心订阅数据面”，没说中心怎么知道有哪些机器、哪台还活着）。

**边界**：网关能做的是**机械聚类**（按路径前缀 / `.app` / 包名去重，给“应用的粗清单”）——那是**视图**，不是清单；拿不到 `software_id` 归一化、版本解析、漏洞挂载。这正是「离线自足」的代价（`agent-purpose-inference.md` §7 已接受）。
