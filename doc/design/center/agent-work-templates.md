...
### 6.2 目录版本：并存 / 逐个迁移 / 物化锁版

`catalog_version` 不是“全局当前值”，而是一条**引用**：

- **多版并存**：`CollectionCatalog` 按 `catalog_version` 各自成版；`superseded_by` 只标“被谁取代”，不删不改历史。
- **模板逐个迁移**：模板的 `catalog_version` 指它按哪版策展，**允许滞后**于最新版；换版 = 改这个字段（走审定），不必所有模板一起动。
- **已授权工作锁版**：`StandingWork.catalog_version` 记物化溯源 —— 目录换版**不追改**在跑的工作；
  要让某台机器跟到新版，走新提案 + 审定。

### 6.3 事实缺位 ≠ 条件不满足

`match` 求值是**三态**：命中 / 不命中 / **无法判定**。包/设备/资源探针未采时是“无法判定”，
不是“没装”（同“空 = 没采不是没装”）。三态分别落 `SelectionBasis`：选中 /
`excluded(match_unsatisfied)` / `excluded(fact_not_collected)`。把后者报成前者，会把“没采”说成“没装”。
