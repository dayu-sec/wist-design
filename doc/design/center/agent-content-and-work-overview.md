...
## 4. 八个关键决策（旧做法 → 为什么改 → 现在怎样）

| # | 决策 | 以前的问题 | 现在 |
|---|---|---|---|
| 1 | **组合，不是继承** | 模板用"单链继承"复用公共内容。可机器类别是**交叉**关系（计算服务器与数据服务器各有专有面，谁也不含谁）→ 一边用继承、一边抄清单，基线一改就悄悄漂移 | 模板 = **平台基线包 ⊕ 特征包**。类别只是"预设选哪几块"，不是继承的根 |
| 2 | **工作粒度 = 面** | 一份工作按"能力"发（只有日志/指标两种），一份日志工作裹了十几个面 → **没法按面暂停/限流** | 一个面 = 一份工作。想停"日志采集里的崩溃面"就停那一份 |
| 3 | **就绪度拆成两义** | `status` 一个字段既表示"规则好了没"，又决定"模板能不能发"→ 一个面就绪也采，整机却被封死 | **规则就绪度**（单元）与**策展成熟度**（包/模板）分开；闸门是**面就绪度**，没就绪的面**单独跳过**，规则好了自动启用（渐进启用） |
| 4 | **目录版本是引用，不是全局开关** | 目录只有一个"当前版"，换版所有模板一起失效 | 多版**并存**；模板可**逐个**迁移；已授权的工作**锁住**（换版不追改，要跟新版走新提案） |
| 5 | **"不知道" ≠ "没有"** | 包探针没采到，`装了 postgresql 吗` 被报成"没装" | 求值**三态**：命中 / 不命中 / **无法判定**。"无法判定"单独记，不冒充"没装" |
| 6 | **来源是结构，不是字符串** | 采集来源是一段自由文本（`glob:A|B|predicate:C`），无法校验，一个串里塞了多个来源也看不出来 | 结构化 `{类型, 取值}`，**一个来源一条**；类型有四种（文件 / 导出器 / 统一日志 / 指标间隔） |
| 7 | **覆盖度要能数出来** | "4 类覆盖约 80%"是拍脑袋的断言 | 新增**机队覆盖度**：各类别台数 + **未归类台数**。要挑下一个类别，从数据里挑 |
| 8 | **用途：规则在网关，模型在中心** | —— | 网关只做**可解释、可测、离线可用**的规则推断；AI/模型只在**中心**，且**只产建议、人工审定**。**网关不引入 AI 依赖** |
